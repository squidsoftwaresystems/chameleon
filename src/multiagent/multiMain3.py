...
class TruckDriver:
  ...
  def updState(self, agent, state, solver):
    ...
    # Didn't use turn it to IDLE bit
  ...
  def utility(self, c):
    ...
    if not self.can_assign_container(c):
      return(score, c.id, c)
    
    # Container is feasible, calculate the score
    score = 0
    ...
    # Distance penalty - can be negative in some instances
    """if c.journey_distance:
      distance_penalty = c.journey_distance * 0.1
      score -= distance_penalty"""

    # Overnight journey check (sleeping cabins and OBUs)
...
class Container:
  ...
  def __str__(self):
    ...
    return f"{self.id}\tType: {self.type}\tWeight: {self.weight}\tADR: {self.adr}\t{route}\t{distance}\t{duration}"
...
class CoCoASolver:
  ...
  def solve(self):
    ...
    # Seems against spirit of non-iterative CoCoA
      self.solveNode(toDo)
      # iterations += 1
      (done, toDo) = self.checkDone(self.agents)
    
    # if iterations >= iteration_limit:
    #  print(f"Warning: Algorithm reached iteration limit of {iteration_limit}")
  ...
  def solveNode(self, agent):
    ...
    for p in agent.prefs:
      #container = p[2]
      #if not container.assigned:  # Only consider unassigned containers
      # and agent.can_assign_container(container):
      ## Filter later, else get containers in thetas that do not have central node added on
